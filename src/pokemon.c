...
{
	HOENN_TO_NATIONAL(COYULA),
	HOENN_TO_NATIONAL(WILYOTE),
	HOENN_TO_NATIONAL(DREAYOLA),
	HOENN_TO_NATIONAL(CAMPEAR),
	HOENN_TO_NATIONAL(REBLRIZZ),
	HOENN_TO_NATIONAL(SMOKARCH),
	HOENN_TO_NATIONAL(BOTOFIN),
	HOENN_TO_NATIONAL(SONARIN),
	HOENN_TO_NATIONAL(GLADIPHON),
	...
	HOENN_TO_NATIONAL(GEOBOI),
	HOENN_TO_NATIONAL(GRAVLOON),
	HOENN_TO_NATIONAL(GOLAEDER),
...
#include "data/pokemon/level_up_learnsets/gen_6.h" // Omg Ruby/Alpha Sapphire
    ...
    for (i = 0; i < ITEM_EFFECT_ARG_START; i++)
    {
        ...
        case 3:
            ...
            if ((itemEffect[i] & ITEM3_PARALYSIS) && HealStatusConditions(mon, STATUS1_PARALYSIS, battlerId) == 0)
                retVal = FALSE;
			if ((itemEffect[i] & ITEM3_BRUISE) && HealStatusConditions(mon, STATUS1_BRUISE, battlerId) == 0)
                retVal = FALSE;
            break;
...
u16 NationalPokedexNumToSpecies(u16 nationalNum)
{
	...
	for (tSpecies=0; tSpecies < NUM_SPECIES-1; tSpecies++){
		if (sSpeciesToNationalPokedexNum[tSpecies] == nationalNum && !(gSpeciesInfo[tSpecies+1].flags & SPECIES_FLAG_SKIP_NAT_DEX)){
			species=tSpecies;
			break;
		}
	}
	...
	//vanilla code
    while ((species < (NUM_SPECIES - 1) && sSpeciesToNationalPokedexNum[species] != nationalNum) || (gSpeciesInfo[species+1].flags & SPECIES_FLAG_SKIP_NAT_DEX))
	{
        species++;
	}
